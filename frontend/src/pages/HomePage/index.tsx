...
import { Send, Bot, User, Activity, FileCheck, CheckCircle2, Circle, Loader2, AlertCircle } from 'lucide-react';

import { contractApi, type ChatMessage, type ChatResponse } from '../../api/client';
...
const HomePage: React.FC = () => {
  ...
  const simulateAIResponse = async (userMessage: string) => {
    ...
    try {
      ...
      // 调用 LLM 对话接口
      const response: ChatResponse = await contractApi.chat(chatMessages);
      ...
      const errorMsg = error.response?.data?.detail || error.message || '无法连接到后端服务器，请检查服务是否启动。';
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {messages.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-gray-400 mt-2">例如："帮我处理一下昨天上传的文件" 或 "解析 test_contract.pdf"</p>
              </div>
            )}
          ...
          <div className="p-6 border-t border-gray-200">
            <div className="flex gap-3">
              <input
                ...
                placeholder={selectedContract ? "输入消息，例如：帮我处理一下这个合同" : "输入消息，例如：帮我处理一下昨天上传的文件"}
                disabled={false}
